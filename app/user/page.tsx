		...
		<div className="min-h-screen bg-slate-50 text-slate-950 dark:bg-slate-950 dark:text-slate-100">
			<div className="max-w-245 mx-auto px-4 pb-28 pt-5">
				{loading ? (
					...
					<div>
						<div>
							<img src={receipt_up_img.src} alt="receipt" />
						</div>

						<div className="bg-white p-6 shadow-2xl shadow-slate-300 dark:bg-slate-950 dark:shadow-slate-900/30">
							<div className="text-center">
								<p className="text-2xl font-semibold text-slate-900 dark:text-slate-100">欢迎使用旅行结算</p>
								<p className="mt-3 text-sm text-slate-500 dark:text-slate-400">为了能同步账单数据，请先登录或注册</p>
							</div>
							...
							{authMode ? (
								<form className="mt-6 space-y-4" onSubmit={authMode === 'login' ? handleLogin : handleRegister}>
									...
									<div className="flex flex-wrap justify-between gap-3">
										<button type="submit" className="w-full max-w-30 rounded-3xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">
						...
						<div>
							<img src={receipt_down_img.src} alt="receipt" />
						</div>
					</div>
				) : (
					<>
						...
						<div className="mt-5 rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950">
							...
							<button type="button" className="flex w-full items-center justify-between gap-4 px-5 py-5 text-left transition hover:bg-slate-50 dark:hover:bg-slate-900">
								<div>
							...
							<button type="button" onClick={logout} className="w-full rounded-3xl bg-rose-600 px-5 py-4 text-sm font-semibold text-white transition hover:bg-rose-500">
								退出登录
							</button>
						</div>
					</>

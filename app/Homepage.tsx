            ...
            <div>
              ...
              <FriendSelector
                ...
                onToggleFriend={(friendId) => {
                  setSelectedFriendsForTrip(prev =>
      ...
      <div className="max-w-245 mx-auto px-4 pb-28 pt-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {currentTrip && (
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      {formatDate(currentTrip.createdAt)}
                    </p>
                  )}
                </div>
        ...
        <section className="mt-4 flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="inline-flex h-10 items-center justify-center rounded-3xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-emerald-500"
          >
            新建账单
          </button>
        </section>
                    ...
                    >
                      <td className="px-3 py-1">
                        <div className="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white font-bold text-xs">
                          {(currentTrip?.members || []).find(m => m.id === bill.payerId)?.name.charAt(0) || '?'}
                        </div>
                      </td>
                      ...
                      <td className="px-3 py-1">
                        <div className="flex flex-wrap items-center gap-1">
                          {bill.owedFriends.slice(0, 4).map((owed: any) => (
                            <div key={owed.id} className="w-6 h-6 bg-gray-500 rounded-full flex items-center justify-center text-white font-bold text-xs">
                              {(currentTrip?.members || []).find(m => m.id === owed.friendId)?.name.charAt(0) || '?'}
                            </div>
                          ))}
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">
                共参加过 {selectedMember.participationCount} 次旅行
              </p>
            </div>

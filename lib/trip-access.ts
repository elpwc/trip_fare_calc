...
const tripInclude = {
  ...
  bills: {
    ...
    select: {
      ...
      owedFriends: {
        where: { isDeleted: false },
        select: { id: true, friendId: true, shareAmount: true, isCustomShare: true },
      },
      name: true,
      category: true,
      status: true,
...
export function formatTripResponse(
  ...
  return {
    ...
    bills: trip.bills.map((bill) => ({
      ...
      owedFriends: bill.owedFriends.map((owed) => ({
        id: owed.id,
        friendId: owed.friendId,
        shareAmount: owed.shareAmount,
        isCustomShare: owed.isCustomShare,
      })),
      name: bill.name,
